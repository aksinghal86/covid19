...
server <- function(input, output) {
    ...
    geo_loc <- reactive({
        # Filter out data
        dat <- all_data %>% filter(state %in% input$state, county %in% input$county)
        peak_date <- create_incidence_objects(dat, peak_date = today()+1)$i.peak
        
        return(list('data' = dat, 'peak_date' = peak_date))
    })
